...
export const VideoLibrary = ({ reload }) => {
  const [videoDetails, setVideoDetails] = useState(null);

  const storedVideoLibrary = localStorage.getItem("videoLibrary")
    ? JSON.parse(localStorage.getItem("videoLibrary"))
    : [];

  useEffect(() => {
    //create an array of promises for fetching movie details
    const fetchVideoDetailsPromises = storedVideoLibrary.map((videoId) => {
      const options = {
        method: "GET",
  ...
  return (
    <section className="w-full min-h-max">
      <h2 className="text-lg mb-2">Video Library</h2>
